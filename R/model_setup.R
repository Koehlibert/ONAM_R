...
get_theta <- function(model_formula, list_of_deep_models) {
  ...
  idx_names <-
    lapply(
      seq_along(theta_deep_name),
      function(idx_item) {
        lapply(
          seq_along(theta_deep_name[[idx_item]]),
          function(idx_item_in) {
            lapply(
              seq_along(theta_deep_name[[idx_item]][[idx_item_in]]),
              function(idx_item_in_2) {
                if (as.character(
                  theta_deep_name[[idx_item]][[idx_item_in]][[idx_item_in_2]])
...
create_models <- function(model_info, inputs_list, list_of_deep_models) {
  ...
  for(idx_p in
    ...
    models_deep[[idx_p]] <-
      lapply(
        seq_along(model_info$name_models[[idx_p]]),
        function(idx_model) {
          name_model <- model_info$name_models[[idx_p]][[idx_model]]
          return(
            list_of_deep_models[[name_model]](
